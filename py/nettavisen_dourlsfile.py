...
def read_article(kilde):
    ...
    article_author = s.find("span", {"class":"byline-author"}, {"itemprop":"name"})
    fulltext=s.find("div", {"class":"article-body"})

    #for EachPart in s.select('p[class*="article--"]'):
    #     fulltext = fulltext + str(EachPart.get_text())

    return date_published.get_text(),article_author.get_text(),fulltext.get_text()
